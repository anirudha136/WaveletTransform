# Q-learning implemantation of GridWorld Game
...
epochs = 5000         #default 3000,significant effect on testing accuracy
gamma = 0.975
epsilon = 1
batchSize = 40
buffer = 80
...
if os.path.isfile(str(epochs)+'_my_model_architecture.json'):
    model = model_from_json(open(str(epochs)+'_my_model_architecture.json').read())
    model.load_weights(str(epochs)+'my_model_weights.h5')
else:
    model = Sequential()
    ...
    open(str(epochs)+'_my_model_architecture.json', 'w').write(json_string)
    model.save_weights(str(epochs)+'my_model_weights.h5')
